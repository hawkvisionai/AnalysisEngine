(() => {
  ...
  function finishBrandIntro() {
    ...
    setTimeout(() => {
      ...
      flight.addEventListener("finish", completeTransition, { once: true });
      flight.addEventListener("cancel", completeTransition, { once: true });

      // iPhone／LINE 內建瀏覽器偶爾不送出 Web Animations 的 finish 事件。
  ...
  // 即使 load、requestAnimationFrame 或動畫事件未正常觸發，
  // 最長 4.2 秒後也一定顯示主畫面。
