...
function enterFromSetup(){if(!validateSetup())return;const returningFromSettings=state.editingSettings;if(state.editingSettings)state.pendingSettingsCommit=true;if(isMemberRole()&&!hasRemainingTime()){renderHours();return}if(state.pendingSettingsCommit)commitSettings();else{state.setupComplete=true;state.actualMode=state.mode||"basic";if(!state.unitPoints)state.unitPoints=calculatedUnit();if(state.mode==="basic")state.noCommission=false;window.HawkVisionAnalysisCore?.setStrategy?.(state.method);window.HawkVisionAnalysisCore?.setLookback?.(requiredGames());queueSave()}showAnalysis();if(returningFromSettings)setRoadVisible(true)}
...
function settleRound(result){
 ...
 if(state.bettingActive&&allowedBefore&&!skip){
   const wager=suggestedBetPoints(),delta=win?(predicted==="莊"?wager*0.95:wager):-wager;
   state.points=Math.max(0,state.points+delta);state.profit+=delta;
