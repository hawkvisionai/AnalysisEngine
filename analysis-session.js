...
function enterFromSetup(){if(!validateSetup())return;const returningFromSettings=state.editingSettings;if(state.editingSettings)state.pendingSettingsCommit=true;if(isMemberRole()&&!hasRemainingTime()){state.pendingSettingsCommit=true;renderHours();return}if(state.pendingSettingsCommit)commitSettings();else{state.setupComplete=true;state.actualMode=state.mode||"basic";if(!state.unitPoints)state.unitPoints=calculatedUnit();if(state.mode==="basic")state.noCommission=false;window.HawkVisionAnalysisCore?.setStrategy?.(state.method);window.HawkVisionAnalysisCore?.setLookback?.(requiredGames());queueSave()}showAnalysis();if(returningFromSettings)setRoadVisible(true)}
