...
function renderSetup(){
 ...
 const activeStyle=(state.family&&STYLE_INFO[state.family]?state.family:methodStyleKey()),i=info(),warn=warningData(),canPoints=!!state.method;
 const card=k=>{const x=STYLE_INFO[k],sel=activeStyle===k;return `<div class="hv-style-card ${sel?"selected":""}" data-style="${k}"><div class="hv-style-head"><b>${x.icon} ${x.name}</b></div><p>${x.brief}</p><div class="hv-levels">${sel?x.methods.map(m=>{const z=methodInfo[m];return `<button type="button" data-method="${m}" class="${state.method===m?"selected":""}">${z.level}</button>`}).join(""):""}</div>${sel&&state.method?`<div class="hv-level-desc">${i.reverse?(i===methodInfo.reverse7?"依目前點數動態調整下注點數，著重點數承受度與穩定性。":"依目前點數動態調整下注點數，在點數運用與收益能力之間取得平衡。"):(k==="tempo"?"依分析訊號控制出手節奏，條件適合時進場。":k==="active"?"提高點數運用與收益潛力，同時承受較高波動。":"依所選層級執行對應的分析與點數控制。")}</div>`:""}</div>`};
 const need=i?(i.reverse?`${fmt(i.recommendedPoints)}點`:`${i.units}注`):"—";
 let html=`<div class="hv-goal-setup"><section class="hv-goal-main"><div class="hv-goal-kicker"><span>分析設定</span> 選擇適合你的玩法風格</div><h2>選擇玩法風格</h2><p class="hv-goal-sub">先依照偏好的出手節奏與點數運用方式選擇風格，再於該風格內選擇層級。</p><div class="hv-style-grid"><div class="hv-style-col">${card("stable")}${card("tempo")}</div><div class="hv-style-col">${card("balanced")}${card("active")}</div></div></section><aside class="hv-goal-side"><h2>本次分析設定</h2><div class="hv-goal-summary"><div><span>玩法風格</span><b>${activeStyle?STYLE_INFO[activeStyle].name:"—"}</b></div><div><span>選擇層級</span><b>${i?i.level:"—"}</b></div><div><span>建議準備</span><b>${need}</b></div></div><label class="hv-point-label">本次帶入點數</label><input id="hvPointsInput" class="hv-point-input" type="text" inputmode="numeric" pattern="[0-9]*" maxlength="10" value="${canPoints?Math.min(CAP,Math.floor(state.points)):""}" ${canPoints?"":"disabled"}><div class="hv-goal-warning ${warn?warn[0]:"empty"}">${warn?`<strong>${warn[1]}</strong><p>${warn[2]}</p>`:""}</div><div class="hv-legend"><i></i><i></i><i></i><i></i></div><div class="hv-goal-bottom"><div class="hv-goal-status">${isMemberRole()?(hasRemainingTime()?"會員・有剩餘時數":"會員・無剩餘時數"):"管理帳號"}</div><button id="hvInlineEnter" type="button" ${setupComplete()?"":"disabled"}>${isMemberRole()&&!hasRemainingTime()?"開啟時數包":"進入分析"}</button></div></aside></div>`;
 ...
 $("hvEntryBody").querySelectorAll("[data-method]").forEach(b=>b.onclick=e=>{e.stopPropagation();state.method=b.dataset.method;const z=info();state.points=methodRecommendedPoints(z);state.initialPoints=state.points;state.profit=0;state.unitPoints=z.reverse?0:100;state.manualEdited=false;renderSetup();requestAnimationFrame(()=>{const input=$("hvPointsInput");if(!input)return;input.focus();try{const n=input.value.length;input.setSelectionRange(n,n)}catch(_){ }if(matchMedia("(max-width:700px)").matches){document.querySelector(".hv-goal-side")?.scrollIntoView({behavior:"smooth",block:"start"})}})});
 ...
 $("hvInlineEnter")?.addEventListener("click",()=>{if(isMemberRole()&&!hasRemainingTime()){renderHours();return}enterFromSetup()});
}
...
function requiredGames(){return info()?.minGames||9}
