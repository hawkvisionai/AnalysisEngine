...
function calculatedUnit(){const i=info();if(!i)return 100;return Math.max(100,Math.floor((Math.max(0,state.points)/i.units)/100)*100)}
...
function displayedSuggested(){if(!state.bettingActive)return "—";if(!currentInternalPrediction())return "本局不下注";if(!coreBetAllowed())return "本局不下注";if(state.points<100||state.skippedSetup)return "—";const wager=suggestedBetPoints();if(wager>state.points)return "本金不足";return `${fmt(wager)} 點`}
...
function renderSetup(){
 beginView("setup");showShell();setErr("");
 ...
 html+=`<section class="hv-setup-section"><h3>選擇分析模式</h3><div class="hv-choice-grid">${choices(modes,"mode",state.mode)}</div></section>`;
 html+=`<section class="hv-setup-section ${state.mode?"":"locked"}"><h3>選擇配注方式</h3><div class="hv-choice-grid two">${state.mode?choices(families,"family",state.family):""}</div></section>`;
 html+=`<section class="hv-setup-section ${state.family?"":"locked"}"><h3>選擇打法模式</h3><div class="hv-choice-grid">${state.family?choices(state.family==="uniform"?uniform:prog,"method",state.method):""}</div></section>`;
 ...
 const canShowPoints=setupComplete()||state.editingSettings;const enterLabel="進入分析";
 html+=`<div class="hv-final-row ${canShowPoints?"":"locked-row"} ${state.mode==="basic"?"basic-mode":""}"><div class="hv-final-cell hv-final-amount"><label>本次帶入點數</label><input id="hvPointsInput" type="text" inputmode="numeric" maxlength="10" value="${canShowPoints?Math.min(CAP,Math.floor(state.points)):""}" ${canShowPoints?"":"disabled"}></div><div class="hv-final-cell hv-final-units"><label>建議準備注數</label><div class="hv-units-split"><strong>${i?i.units+" 注":"—"}</strong><strong class="hv-one-bet">一注 ${i?fmt(state.editingSettings?calculatedUnit():baseUnit()):"—"} 點</strong></div></div><div class="hv-final-cell hv-final-commission ${state.mode==="basic"?"hv-hidden-basic":""}"><label>免佣</label><button id="hvCommissionSetup" class="hv-commission-toggle ${state.noCommission?"on":""}" type="button" aria-pressed="${state.noCommission}"><span class="on-label">ON</span><span class="off-label">OFF</span></button></div><button id="hvInlineEnter" class="hv-final-enter" type="button" ${setupComplete()?"":"disabled"}>${enterLabel}</button></div></div>`;
 ...
 $("hvSetupClose")?.addEventListener("click",()=>{if(state.editingSettings||state.entered){if(state.editingSettings)restoreSettingsSnapshot();state.editingSettings=false;showAnalysis();return}state.mode="basic";state.family="uniform";state.method="standard";state.points=0;state.initialPoints=0;state.profit=0;state.unitPoints=0;state.noCommission=false;state.skippedSetup=true;state.setupComplete=true;window.HawkVisionAnalysisCore?.setStrategy?.("standard");showAnalysis();saveRuntime().catch(()=>{})});
}
function renderDynamicSetupBits(){const i=info();const u=document.querySelector(".hv-one-bet");if(u)u.textContent=`一注 ${fmt(calculatedUnit())} 點`;const slot=document.querySelector(".hv-warning-slot");if(slot){const w=warningData();slot.className=`hv-setup-section hv-warning-slot ${w?`active ${w[0]}`:"locked"}`;slot.innerHTML=w?`<div class="hv-warning-copy"><strong>${w[1]}</strong><p>${w[2]}</p></div>`:""}}
...
async function renderHours(){const viewEpoch=beginView("hours");showShell();setErr("");if(isMemberRole())await loadHours().catch(e=>{if(isCurrentView(viewEpoch,"hours"))setErr(e.message||String(e))});if(!isCurrentView(viewEpoch,"hours"))return;$("hvEntryTitle").innerHTML=brandTitle(`<span class="hv-hours-top-right"><span class="hv-used-total">${isMemberRole()?`已使用總時數 <b>${state.usedTotal}</b> 小時`:""}</span>${isMemberRole()?`<button id="hvHoursLogout" class="hv-top-close" type="button">登出</button>`:""}<button id="hvHoursEnter" class="hv-top-close" type="button">返回分析</button></span>`);$("hvEntryHint").textContent="";["hvEntryBack","hvEntrySkip","hvEntryNext"].forEach(id=>$(id).style.display="none");const packs=state.hourInventory.filter(p=>Number(p.available_count)>0);const packHtml=isManagementRole()?`<div class="hv-hours-unlimited">不受時數限制</div>`:(packs.length?packs.map(p=>{const h=Number(p.hours_per_package),count=Number(p.available_count),qty=Math.max(1,Math.min(count,state.packQty[h]||1));state.packQty[h]=qty;return `<div class="hv-hour-row-wrap"><div class="hv-hour-card"><div><strong>${h} 小時</strong><small>可使用 ${count} 包</small></div><div class="hv-pack-actions"><div class="hv-inline-stepper" aria-label="開啟數量"><button type="button" data-pack-minus="${h}" ${qty<=1?"disabled":""}>−</button><strong>${qty}</strong><button type="button" data-pack-plus="${h}" ${qty>=count?"disabled":""}>+</button></div><button class="hv-hour-open" data-hour="${h}" data-max="${count}" type="button">開啟</button></div></div></div>`}).join(""):`<div class="hv-hours-none">沒有可用時數包，請聯絡上層管理</div>`);$("hvEntryBody").innerHTML=`<div class="hv-time-panel"><div class="hv-time-caption">目前剩餘時間</div><div id="hvHoursLiveTime" class="hv-time-big">${isManagementRole()?"不受限制":formatTime(calcRemaining())}</div></div><section class="hv-hours-available"><h3>可使用時數包</h3><div class="hv-hour-list">${packHtml}</div></section>`;$("hvHoursEnter")?.addEventListener("click",()=>{if(state.pendingSettingsCommit)commitSettings();showAnalysis()});$("hvHoursLogout")?.addEventListener("click",async()=>{await window.hvGlobalLogout?.()});if(isMemberRole()){$("hvEntryBody").querySelectorAll("[data-pack-minus]").forEach(b=>b.onclick=()=>{const h=Number(b.dataset.packMinus);state.packQty[h]=Math.max(1,(state.packQty[h]||1)-1);renderHours()});$("hvEntryBody").querySelectorAll("[data-pack-plus]").forEach(b=>b.onclick=()=>{const h=Number(b.dataset.packPlus),p=state.hourInventory.find(x=>Number(x.hours_per_package)===h);state.packQty[h]=Math.min(Number(p?.available_count||1),(state.packQty[h]||1)+1);renderHours()});$("hvEntryBody").querySelectorAll(".hv-hour-open").forEach(b=>b.onclick=()=>{const h=Number(b.dataset.hour),max=Number(b.dataset.max);openHourConfirm(h,state.packQty[h]||1,max)})}}
...
function syncStartControls(){const req=requiredGames(),effective=Math.min(effectiveRoundCount(),req),ready=effective>=req&&!document.body.classList.contains("hv-time-locked");if(effective<req&&(state.analysisActive||state.bettingActive)){state.analysisActive=false;state.bettingActive=false;state.progressionIndex=0;state.officialHistory=[];resetSkip();window.HawkVisionAnalysisCore?.deactivateAnalysisPreserveShoe?.();if($("hvSuggestedBet"))$("hvSuggestedBet").textContent="—"}const a=$("hvStartAnalysis"),b=$("hvStartBetting"),hint=$("hvRequiredGames");if(hint){const total=Math.max(0,Number($("roundCount")?.textContent||0));hint.innerHTML=`<span>輸入有效局數 ${effective}/${req}</span><span class="hv-mobile-total">總局數 ${total}</span>`;}if(a){a.textContent=state.analysisActive?"分析中":"開始分析";a.disabled=!ready||state.bettingActive||state.analysisActive}if(b){b.textContent=state.bettingActive?"下注中":"開始下注";b.disabled=!ready||state.bettingActive}}
...
function settleRound(result){
 ...
 if(!signal){resetSkip();if($("hvSuggestedBet"))$("hvSuggestedBet").textContent=displayedSuggested();queueSave();return}
 ...
 if(state.bettingActive&&allowedBefore&&!skip){
   ...
   if(wager<=state.points){
     ...
     const t=progressionType();
     if(t==="loss")state.progressionIndex=win?0:(state.progressionIndex>=4?0:state.progressionIndex+1);
...
window.HawkVisionSessionPolicy={
 isPredictionPublic(){return coreBetAllowed()},
